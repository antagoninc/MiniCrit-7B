...
class ModelLoader:
    ...
    def generate(self, input_text: str, domain: str = "general") -> tuple[str, float, int]:
        ...
        with torch.no_grad():
            outputs = self.model.generate(  # type: ignore[union-attr,attr-defined]
                **inputs,
                max_new_tokens=256,
                do_sample=True,
                temperature=0.7,
                top_p=0.9,
                pad_token_id=self.tokenizer.pad_token_id,  # type: ignore[union-attr,attr-defined]
                eos_token_id=self.tokenizer.eos_token_id,  # type: ignore[union-attr,attr-defined]
            )
        ...
        # Decode
        generated = self.tokenizer.decode(  # type: ignore[union-attr,attr-defined]
            outputs[0][input_len:],
        ...
        by_type[example.data_type].append(
            {
                ...
                "output": output,
                "is_false_positive": (
                    is_false_positive(critique) if example.data_type == "hard_negative" else None
                ),
            }
        )
...
def compute_benchmark_result(
    ...
    for dtype, items in by_type.items():
        detection_by_type[dtype] = {
            "count": len(items),
            "avg_latency_ms": (
                statistics.mean([x["output"].latency_ms for x in items]) if items else 0
            ),
        }
...
def generate_report(
    ...
    summary = {
        ...
        "comparison": {
            ...
            "fp_rate_improvement": (
                f"{((result_1.false_positive_rate - result_2.false_positive_rate) / result_1.false_positive_rate * 100):.1f}%"
                if result_1.false_positive_rate > 0
                else "N/A"
            ),
        },
    }
    ...
    print(
        f"{'Avg Latency (ms)':<30} {result_1.avg_latency_ms:<20.1f} {result_2.avg_latency_ms:<20.1f}"
    )
    print(
        f"{'P95 Latency (ms)':<30} {result_1.p95_latency_ms:<20.1f} {result_2.p95_latency_ms:<20.1f}"
    )
    print(
        f"{'False Positive Rate':<30} {result_1.false_positive_rate:<20.2%} {result_2.false_positive_rate:<20.2%}"
    )
    print(
        f"{'Peak Memory (GB)':<30} {result_1.peak_memory_gb:<20.1f} {result_2.peak_memory_gb:<20.1f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--judge-sample", type=int, default=50, help="Number of examples for LLM judge"
    )
    ...
    if not args.skip_judge and os.environ.get("ANTHROPIC_API_KEY"):
        ...
        judge_examples = random.sample(
            list(zip(examples, outputs_1, outputs_2)), min(args.judge_sample, len(examples))
        )
